 ...
 */
	...
	start_PVSio_process(pid_of_son,true);
    ...
    index_state=findVariable("xx",state);
    if ( index_state != -1){
		writeOutputVariableDouble(index_state,6);
	}
	
	
    index_state=findVariable("yy",state);
    if ( index_state != -1){
		writeOutputVariableDouble(index_state,7);
	}
	
	
    index_state=findVariable("theta",state);
    if ( index_state != -1){
...
void start_PVSio_process(int& pid, bool first){
	pipe(FtS);
    pipe(StF);
	 pid_of_son = fork();
	if ( pid_of_son == 0) {
	...
	fd=fdopen(StF[0], "r");
	sprintf(sendbuff,"init_ext_state;");
	if (first==true)write(FtS[1], sendbuff,strlen(sendbuff));
}
...
void restart_PVSio(){
	...
	sprintf(sendbuff,"pkill -9 -s %d\n",pid_of_son);
	system(sendbuff);
	start_PVSio_process(pid_of_son,false);
...
void retrieveExternalVariables(){
	 memset(externalvariables,0,MAX_MEMORY); // we need to clean the variables before reusing them
	    ...
	    }if( findVariable("xx", state)== -1){
			if ( strlen(externalvariables) == 0) {
				sprintf(tempvar,";(# xx := %f\n",fmiBuffer.realBuffer[6]); 
				strcat(externalvariables,tempvar);
				}
			else {
				sprintf(tempvar,",xx := %f\n",fmiBuffer.realBuffer[6]);
	    ...
	    }if( findVariable("yy", state)== -1){
			if ( strlen(externalvariables) == 0) {
				sprintf(tempvar,";(# yy := %f\n",fmiBuffer.realBuffer[7]); 
				strcat(externalvariables,tempvar);
				}
			else {
				sprintf(tempvar,",yy := %f\n",fmiBuffer.realBuffer[7]);
	    ...
	    }if( findVariable("theta", state)== -1){
			if ( strlen(externalvariables) == 0) {
				sprintf(tempvar,";(# theta := %f\n",fmiBuffer.realBuffer[8]); 
				strcat(externalvariables,tempvar);
				}
			else {
				sprintf(tempvar,",theta := %f\n",fmiBuffer.realBuffer[8]);
...
void writeOutputVariableDouble(int state_index, int buffer_index){
	...
	if(atof(&state[state_index]) == 0){
		...
		while(state[state_index+offset] != ','  && state[state_index+offset] != ' ' && state[state_index+offset] != '/' && state[state_index+offset] != 'E' && state[state_index+offset] != 'e')offset++;
		switch(state[state_index+offset]){
			case 'e' :
				...
				readInputVariableDouble(state_index,temp1,buffer_index);
			break;
			case 'E' : 
				sscanf(&state[state_index],"%[^'E']E%s %*s",r1,r2);
				temp1=atof(r1);
